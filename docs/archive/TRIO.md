...
### Workflow-Based TRIO Variant

The sprint stage can be driven by the `wave-dispatch` workflow instead of manual orchestration:

**What the workflow automates:**
- Splitting tests into waves by file independence
- Spawning up to 3 coders per wave in parallel worktrees (`isolation: 'worktree'`)
- GREEN gate with retries (max 3)
- Post-wave gates: wiring, visual, wave-smoke
- Hidden gate after all waves
- Alignment gate with exit-code routing
- Patch waves (max 2)
- Wiring + activation proofs
- Pipeline advancement via gate.sh

**What stays manual (skill-driven):**
- Test generation (RED gate) — delegated to `sdd-test-gen` workflow
- Review — delegated to `sdd-review` workflow
- Retro — delegated to `sdd-retro` workflow

**How to trigger:**
- `/sdd <feature>` — automatically delegates to workflows
- `ultracode: implement <spec>` — uses `sdd-implement` meta-workflow
- `/trio <feature>` — delegates Phase 2 to `wave-dispatch` workflow
